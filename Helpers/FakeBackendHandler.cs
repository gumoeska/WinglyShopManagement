...
namespace WinglyShopAdmin.App.Helpers
{
    public class FakeBackendHandler : HttpClientHandler
    {
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            else if (path == "/users/register" && method == HttpMethod.Post)
            {
            ...
            {
                var bodyJson = await request.Content.ReadAsStringAsync();
                var body = JsonSerializer.Deserialize<Tuple<string, string, string>>(bodyJson);
                var user = users.FirstOrDefault(x => x.Username == body.Item1);

                if (user != null)
                    return await error("Conta já Existe");
